...
xemo.plugins.Login.prototype.init = function () {
    ...
    xemo.core.sopv2({
        op:    'personnel.get.usernames'
    }, function (data) {
        for (var x = 0; x < data.length; ++x) {
            var username = data[x].username;
            $(self.username).append('<option value="' + username + '">' + username + '</option>');
        }
    });
    ...
    this.userpwsubmit.onclick = function () {
        ...
        var hash = CryptoJS.SHA512('{0}:{1}'.format(
            halflogin ? 'anybody' : this.__loginobj.username.value, 
    ...
    */
    ...
    this.can_write = function () {
        return this.canwrite;
    };

    this.can_read = function () {
        return this.canread;
    }
    ...
    this.verify = function (hash, cb) {
        ...
        xemo.core.sop({
            ...
            self.canread = true;
            self.canwrite = data.perm ? true : false;
    ...
    this.doVerify = function (hash) {
        ...
        this.verify(hash, function (result, id, username, error_text) {
            ...
            if (result != undefined) {
                ...
                __obj.valid_username = result.username;
                __obj.valid_passhash = hash;
                __obj.valid_pid = result.id;

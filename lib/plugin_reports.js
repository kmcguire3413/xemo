...
xemo.plugins.Reports.mods.Timesheet = function (reports, frame, params) {
    ...
    var from_pp = xemo.core.getpayperiodidbydate(from, 0);
    if (from != from_pp[0]) {
    ...
    var to_pp = xemo.core.getpayperiodidbydate(to, 0);
    if (to != to_pp[1]) {
    ...
    alert('chop-begin: ' + from_pp + ' chop-end: ' + to_pp);

    for (var i = 0; i < shifts.length; ++i) {
        var shift = shifts[i].excludeafter(to_pp);
        if (shift == null) {
            continue;
        }
        shift = shift.excludebefore(from_pp);
        if (shift == null) {
            continue;
        }
    ...
    for (var i = 0; i < local.length; ++i) {
        ...
        // This is broke when straddling a timezone change.
...
xemo.plugins.Reports.mods.DayListWithBars.prototype.generateReport = function (colormode_specified) {
    ...
    $(mw.document.head).append('<link rel="stylesheet" type="text/css" href="http://kmcg3413.net/themes/default/master.css"/>');
    $(cont).append(tbl);

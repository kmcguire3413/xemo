...
xemo.plugins.Reports.mods.Timesheet = function (reports, frame, params) {
    ...
    function generate_timesheet(ppstart, pid, data) {
        ...
        xml['payperiodstart'] = ppstart.shortString();
        var ppend = ppstart.clone();
        ppend.setDate(ppend.getDate() + 7);
        xml['payperiodend'] = ppend.shortString();
    ...
    genbtn.onclick = function () {
        for (var x = 0; x < opts.length; ++x) {
            if (opts[x].checked) {
                ...
                generate_timesheet(ppdate, pid, dmonth[monthid].data[pid]);
            }
        }
    }

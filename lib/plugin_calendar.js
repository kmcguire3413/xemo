...
xemo.CalendarModules.PayCalc.prototype.update = function () {
    ...
    plugin_pay_bootstrap(
        ...
        function (master) {
                ...
                if (tmp) {
                    ...
                    if (tmp) {
                        if (shift.pid == 2) {
                            ...
                            if (tmp_sum > 0) {
                                //tmp_play += '<tr><td>' + tmp_sum + '</td><td>' + shift.start + '</td><td>' + shift.end + '</td></tr>';
                        ...
                        if (!monthly_tally[shift.pid][shift.allocatedfor.id]) {
                            monthly_tally[shift.pid][shift.allocatedfor.id] = 0;
                        }

                        if (tmp.pid == 2) {
                            //tmp_play += '<tr><td>' + shift.allocatedfor.id + '</td><td>' + tmp_hours + '</td><td>' + tmp.start + '</td><td>' + tmp.end + '</td></tr>';
                            //console.log('HERE', shift.allocatedfor.id, tmp_hours, tmp.start, tmp.end);
                        }
                        monthly_tally[shift.pid][shift.allocatedfor.id] += tmp_hours;
                    }
                }                
            ...
            console.log('monthly_tally', monthly_tally);
            console.log('tmp_play', tmp_play);


            //$(self.extraframe).empty();
            //$(self.extraframe).append('<table>' + tmp_play + '</table>');
            ...
            for (var pid in monthly_tally) {
                ...
                out.push('<table style="border: 1px gray solid;">')
                out.push('<thead style="border-collapse: collapse;"><td>Name</td>');
                for (var aid in monthly_tally[pid]) {
                    out.push('<td style="font-weight: bold; border: 1px black solid;">' + aid + '</td>');
                }
                out.push('</thead><tr><td style="border: 1px gray solid;">Leonard Kevin McGuire Jr.</td>');
                for (var aid in monthly_tally[pid]) {
                    out.push('<td style="border: 1px gray solid;">' + monthly_tally[pid][aid] + '</td>');
                }
                out.push('</tr></table>')
            }

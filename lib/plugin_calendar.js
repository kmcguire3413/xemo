...
xemo.CalendarModules.EFDHoursTally.prototype.update = function () {
    ...
    for (var x = 0; x < shifts.length; ++x) {
        ...
        //console.log('DEBUG', delta, shifts[x].name, shifts[x].start, shifts[x].end);

        if (!shifts[x].isValid()) {
            warnings['shift#' + x] = 'The shift of ' + shifts[x].start + ' is flagged as invalid. Check times?'
            ++warning_count;
        }
    ...
    //console.log('matched_totals', matched_totals);
    ...
    for (var name in matched_totals) {
        ...
        var out = ['<tr><td>', name.toUpperCase(), '</td><td>', hours_round(matched_totals[name]), '</td></tr>'];
        $(tbl).append(out.join(''));
    }
    $(notesframe).append(tbl);    
};
...
xemo.CalendarModules.PayCalc = function (plugin) {
    ...
    this.extraframe = document.createElement('div');
    ...
    $(this.subframe).append(this.extraframe);
    ...
    plugin_pay_bootstrap(
        ...
        function (master) {
            ...
            for (var x = 0; x < master.shifts.length; ++x) {
                ...
                pidtoname[shift.pid] = shift.name;

                var tmp = shift.excludebefore(new Date(self.plugin.cur_year, self.plugin.cur_month - 1, 1));
                if (tmp) {
                    ...
                    tmp = tmp.excludeafter(new Date(self.plugin.cur_year, self.plugin.cur_month, 1, 6));

                    if (tmp) {
                        var payperiodref = new Date(master.paysysinfo.ppref[shift.pid] * 1000);
                        var ppresult = xemo.core.getpayperiodidbydate(shift.start, 2, payperiodref)[1][0];
                        result = ppresult.getTime();   

                        var shift_hours = (shift.end.getTime() - shift.start.getTime()) / 1000 / 60 / 60;

                        if (shift.pid == 2) {
                            console.log('@@@' + tmp.start + ' ' + tmp.end);
                            console.log('-->' + shift_hours);
                            console.log('-->' + shift.allocatedfor.id);
                            console.log('pp: ' + ppresult);
                        }

                        /*
                            TODO: fix this hard coded logic

                            A summary of hours between 1500 and 1800 for one personnel.
                        */
                        if (shift.pid == 2) {
                            var tmp_sum = tmp.countHoursBetweenHoursBetweenWeekdays(15, 18, 1, 5);
                            if (tmp_sum > 0) {
                                heather_tally.push([tmp_sum, tmp.start.shortString(), tmp.end.shortString()]);
                            }
                        }

                        var tmp_hours = (tmp.end.getTime() - tmp.start.getTime()) / 1000 / 60 / 60;

                        if (!monthly_tally[shift.pid]) {
                            monthly_tally[shift.pid] = {};
                        }                     


                        if (!monthly_tally[shift.pid][result]) {
                            monthly_tally[shift.pid][result] = {};
                        }

                        if (!monthly_tally[shift.pid][result][shift.allocatedfor.id]) {
                            monthly_tally[shift.pid][result][shift.allocatedfor.id] = 0;
                        }

                        monthly_tally[shift.pid][result][shift.allocatedfor.id] += tmp_hours;
                    }
                }                
            ...
            $(self.extraframe).empty();

            /*
                The following code builds the table that displays the totals for all types of hours
                worked by personnel. It also attempts to produce a gross value in dollars. The dollar
                calculate really needs to be set by the hours allocators as a parameter or sub-parameter
                of the shift object; however, for now, it is done here.
            */

            function build_table(nt) {
                // Enumerate all types of columns.
                var cols = [];
                var ndx = 0;
                ...
                out.push('<table style="border: 1px solid gray;">');
                ...
                for (var k in cols) {
                    hdr[cols[k]] = '<td style="border: 1px solid gray;">' + k + '</td>';
                }

                out.push('<thead>' + hdr.join('') + '</thead>');
                ...
                for (var rec in recs) {
                    ...
                    out.push('<tr>');
                    for (var k in cols) {
                        out.push('<td style="border: 1px solid gray;">' + rec[k] + '</td>');
                    }
                    out.push('</tr>');
                }
            ...
            /*
                Here is the shifts which were tallied above are added and valued at cost then
                this compiled into a special table. This special table is converted into an
                HTML formed table with static columns.
            */
            for (var pid in monthly_tally) {
                for (var pp in monthly_tally[pid]) {
                    var tmp_name;

                    tmp_name = pidtoname[pid];

                    var pptxt = new Date(parseInt(pp));

                    var gross = 0.0;

                    var nte = {
                            "NAME":           tmp_name,
                            "PAY PERIOD <I>OF MONTH</I>":     pptxt,
                    };

                    for (var aid in monthly_tally[pid][pp]) {
                        var value = 0.0;

                        switch (aid) {
                            case "8x5 PAY": value = 8.0; break;
                            case "STANDARD INCENTIVE": value = 2.0; break;
                            case "PART29 PAY": value = 8.0; break;
                        }

                        nte[aid] = hours_round(monthly_tally[pid][pp][aid], 1);

                        gross += hours_round(monthly_tally[pid][pp][aid]) * value;
                    }

                    nte["GROSS PAY"] = Math.round(gross, 2);

                    nt.push(nte);
                }          
            }

            //$(self.extraframe).append(out.join(''));

            $(self.extraframe).append(build_table(nt));
            $(self.extraframe).append('<I>The table only reflects hours on <u>this</u> month which start and stop at the pay period boundary in hh:mm for each personnel. The next month or previous month must be looked at for complete hour total for pay periods that span the boundary of the month.</i>');
            ...
            var __pad = function (pad, str, padLeft) {
            ...
            var __repeat = function(str, count) {
                var array = [];
                for(var i = 0; i <= count;)
                    array[i++] = str;
                return array.join('');
            ...
            var txt = 'KEVIN HOURS WITH SHIFTS BETWEEN 1500 AND 1800 ON MON THRU FRI\n' +
                      __pad(__repeat(' ',10), 'HOURS') +
                      __pad(__repeat(' ',16), 'SHIFT-START') +
                      __pad(__repeat(' ', 16), 'SHIFT-END') + '\n';

            for (var x = 0; x < heather_tally.length; ++x) {
                var e = heather_tally[x];
                var hours = e[0];
                var st = e[1];
                var et = e[2];

                txt += __pad(__repeat(' ', 10), hours) + 
                       __pad(__repeat(' ', 16), st) + 
                       __pad(__repeat(' ', 16), et) + '\n';
            }

            console.log(txt);

            $(self.notesframe).append('<pre>' + txt + '</pre>');

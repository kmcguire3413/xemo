...
xemo.plugins.Admin.prototype.init = function () {
    ...
    var tk = this.tk;
    for (var k in xemo.core.wizards) {
        var wizard = xemo.core.wizards[k];
        if (wizard.cat != 'admin') {
            continue;
        }
        ...
        link.onclick = function () {
            this.wizard.open( { tk: tk, pframe: pframe} );
        }
...
xemo.core.dialogs = {
    ...
    NewDialog: function (state, myframe) {
        if (myframe == undefined) {
            ...
            backbtn.onclick = function () {
                xemo.core.dialogs.CloseDialog(state);
            };
    ...
    */
    CloseDialog: function (state) {
        if ($(state.pframe).children().last().noremove) {
            return;
        }
...
xemo.core.wizards.EditPersonnel = {
    cat:          'admin',
    description:  'I want to change something about a personnel.',
    open:  function (state) {
        xemo.core.dialogs.PickPersonnel(state, function (pid, pdata){
            xemo.core.dialogs.EditPersonnel(state, pid, function (tocommit) {
                xemo.core.dialogs.SavePersonnel(state, tocommit, function () {
                    xemo.core.dialogs.ShowSuccess(state,
...
xemo.core.wizards.AddPersonnel = {
    cat:          'admin',
    description:  'I want to add a new personnel to the system.',
    open: function (state) {
        xemo.core.dialogs.EditPersonnel(state, -1, function (tocommit) {
            xemo.core.dialogs.SavePersonnel(state, tocommit, function () {
                xemo.core.dialogs.ShowSuccess(state, 

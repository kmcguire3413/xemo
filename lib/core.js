...
Date.prototype.subtractTimeConsideringTimezone = function (a) {
    if (this.getTime() < a.getTime()) {
        xemo.core.waitError('Opps.. looks like we have a time subtraction issue..');
    }
...
function system_hourly_lunchcarve(params) {
    ...
    while (need > 0) {
        for (var x = 0; x < shifts.length; ++x) {
            ...
            if (shift.start.getDay() < stddaysbegin && shift.start.getDay() > stddaysend) {
                continue;
            }
            ...
            var hours = shift.end.subtractTimeConsideringTimezone(shift.start) / 1000 / 60 / 60;
            ...
            if (result.length > 0) {
                ...
                picked.allocatedfor = allocatedfor;
                allocated.push({
...
function system_hourly_withcap(params) {
    ...
    function __adaptive(allocated, totalhours, pps, ppe) {
        ...
        xemo.core.log(10, 'fiddle set to ' + fiddle + '<br/>');
        ...
        xemo.core.log(10, 'SLACK IS ' + slack);
        ...
        for (var x = 0; x < dayshifts.length; ++x) {
            ...
            while ( ((level & 1) * slack) + (level >> 1) <= dayhours[x] ) {
                ...
                xemo.core.log(10, '  level:{0} windowsize:{1}'.format(level, windowsize));
                ++level;
                if (windowsize == 0) {
                    continue;
                }
                //xemo.core.log(10, 'dayhours[{0}]: {1}'.format(x, dayhours[x]));
                ...
                for (var offset = 0; offset <= maxoffset; ++offset) {
                    function __offsetIntoShiftArrayForLength(sarray, offset, windowsize) {
                        ...
                        function __offsetIntoShiftArray(sarray, offset, z) {
                            if (offset > 0) {
                                ...
                                for (; z < sarray.length; ++z) {
                                    ...
                                    if (result[0] == 1) {
                                        //xemo.core.log(10, '     @ ate whole shift and done');
                                    ...
                                    if (result[0] == 2) {
                                        //xemo.core.log(10, '     @ ate partial amount of shift');
                                    ...
                                    // More to go..
                                    offsetrem -= result[1];
                                    //xemo.core.log(10, '     @ ' + offsetrem + ' more to go');
                                }
                        ...
                        //xemo.core.log(10, 'doing first offset into shift array');
                        //xemo.core.log(10, 'offset: ' + offset);
                        //xemo.core.log(10, 'windowsize: ' + windowsize);
                        var result1 = __offsetIntoShiftArray(sarray, offset, 0);
                        //xemo.core.log(10, '    got offset of ' + result1[1]);
                        //xemo.core.log(10, '    doing second offset into shift');
        ...
        for (var x = 0; x < dayhours.length; ++x) {
            xemo.core.log(10, '     dayhours[{0}]: ' + dayhours[x]);
        }

        xemo.core.log(10, 'shiftslidecache dump ' + pps);
        for (var x = 0; x < shiftslidecache.length; ++x) {
            for (var y in shiftslidecache[x]) {
                for (var z in shiftslidecache[x][y]) {
                    xemo.core.log(10, 'x:{0} windowsize:{1} offset:{2} value:{3}'.format(
        ...
        xemo.core.log(10, 'trying combinations');
        ...
        xemo.core.log(10, '    best_hours:{0} best_transports:{1} worst_transports:{2} trycount:{3}'.format(
            best_hours,
            best_transports,
            worst_transports,
            trycount
        ...
        */
            ...
            for (var x = 0; x < p.length; ++x) {
                ...
                for (var y = 0; y < dayshifts[ndx].length; ++y) {
                    ...
                    if (totalshifthours > need) {
                        ...
                        if (!pretend) {
                            xemo.core.log(10, 'B. took {0} hours from {1} to {2}<br/>'.format(need, shift.start, shift.end));
                        ...
                        if (!pretend) {
                            xemo.core.log(10, 'B. took all hours from {0} to {1}<br/>'.format(shift.start, shift.end));
                            allocated.push({
                                'start':          shift.start.clone(),
        ...
        permutate(map, function(p) {
            ...
            if (bestcount == -1 || (avoidspecial && (transportcount < bestcount))) {
                if (resulthours > besthours) {
                    ...
                    bestmap = p.slice();
                    xemo.core.log(10, 'set-best-map:' + bestmap.join(','));
                    xemo.core.log(10, 'resulthours: ' + resulthours);
                    return;
                }
            } 
        ...
        xemo.core.log(10, 'DBG-CHECK:' + result[1]);
    ...
    function __firstfind(allocated, totalhours, pps, ppe) {
        ...
        for (var i = 0; i < shifts.length; ++i) {
            ...
            xemo.core.log(10, 'A. for pid {0} index:{1} looking at shift {2} ... {3} to {4}<br/>'.format(pid, i, shifts[i].name, shifts[i].start, shifts[i].end));
            ...
            //if (shifts[i].start.getHours() == 6 && shifts[i].start.getDay() >= 1 && shifts[i].start.getDay() <= 5) {
                ...
                if (totalshifthours > need) {
                    ...
                    ++i;
                    xemo.core.log(10, 'removed {0} hours from shift {1} to {2}<br/>'.format(need, shifts[i].start, shift_end));
                } else {
                    totalhours += totalshifthours;
                    xemo.core.log(10, 'removed entire shift {0} to {1}<br/>'.format(shifts[i].start, shift_end));
                    // We are going to just remove the entire shift.
                    allocated.push({
                        'start':          shifts[i].start.clone(),
                        'end':            shifts[i].end.clone(),
...
xemo.core.paysys = {
    'DIRECTOR': function (params) {
        ...
        var payperiodref = new Date(specs.payperiodref * 1000);
        ...
        */
        ...
        // C shift is pp14index = 0 and first day of pay period
        //
        // 3 for ABC
        // 1 for offset (C shift is pp14index = 0)
        // 4 for offset since thursday is day index 4
        var ashiftndx = xemo.core.modulus(5 - pp7index, 7);

        //(ashiftndx)           ANY OVER 6 THEN DISCARD
        //(ashiftndx + 3)
        //(ashiftndx + 6)

        var hourcap;
        if (xemo.core.modulus(pp7index, 3) == 0) {
            hourcap = 72;
        } else {
            hourcap = 48;
        }
        ...
        var param_normal = {
            ...
            'stddayfunc':     function (start) {
                var acday = start.getDay();
                if (start.getHours() < 8) {
                    acday = xemo.core.modulus(acday - 1, 7);
                }
                switch (acday) {
                    case ashiftndx: return true;
                    case xemo.core.modulus(ashiftndx + 3, 7): return true;
                    case xemo.core.modulus(ashiftndx + 6, 7): return true;
                }
                return false;
            },
            'avoidspecial':     false,
            ...
            'note':           'STANDARD PAY A-SHIFT-DAYS({0},{1},{2})'.format(
                ashiftndx, ashiftndx + 3, ashiftndx + 6
            )
        };
        ...
        var param_over = {
            ...
            'note':           'OVERTIME PAY A-SHIFT-DAYS({0},{1},{2})'.format(
                ashiftndx, ashiftndx + 3, ashiftndx + 6
            )
        };
    ...
    'DRIVER_8X54X6': function (params) {
        ...
        // Try to allocate lunch hours.. only on monday thru friday..
        var lunchret = system_hourly_withcap(param_hourslunch);
        ...
        if (hours5x8 < 40) {
            ...
            for (var x = 0; x < lunchret.shifts.length; ++x) {
                lunchret.shifts[x].__shift.allocatedfor = null;
            }
            ...
            // Reallocate lunch hours if any are possible..
            var lunchret = system_hourly_withcap(param_hourslunch);
...
xemo.core.splitshiftsby = function (shifts, by) {
    for (var i = 0; i < shifts.length; ++i) {
        if (xemo.core.splitshiftinplaceby(shifts, i, by)) {
            xemo.core.log(10, 'splitshiftsby for {0} on {1} and {2}'.format(
...
function plugin_pay_calculator(paysysinfo, pids, nonpidnames, calls, shifts, key, from, to, cfg, cb) {
    var master = {};
    ...
    */
    for (var i = 0; i < shifts.length; ++i) {
        ...
        if (!shift.pid) {
            ...
            payperiodref = new Date(paysysinfo.systems[paysysinfo.mapping[shift.pid]].payperiodref * 1000);
        }
...
xemo.core.getPersonnelIDFromName = function (pdata, name, reldate) {
    ...
    if (name in xemo.core.getPersonnelIDFromName.cache) {
        return [1, xemo.core.getPersonnelIDFromName.cache[name]];
    }
    ...
    xemo.core.getPersonnelIDFromName.cache[name] = foundpid;
    return [1, foundpid];
}
xemo.core.getPersonnelIDFromName.cache = {};

xemo.core.opendoc = function (key, ext, contents) {
    $.post('v1api.py', { key: key, op: 'gen_document', data: contents, ext: ext}, function (data) {
        var docrelurl = $.parseJSON(data).result;
        var mw = window.open('/fcal/' + docrelurl);
    }).fail(function () {
        xemo.core.waitError('A document requested could not be opened!');
    });
}
...
xemo.core.fetchPersonnelData = function (key, cb) {
    $.get('v1api.py?key={0}&op=get_personnel_data'.format(key), function (data) {
        data = $.parseJSON(data).result;
        cb(data);
    }).fail(function () {
        xemo.core.waitError('We failed to fetch personnel data from database..');
        cb(undefined);
    });
}
...
xemo.core.getPersonnelNamesFromIDs = function (key, ids, cb) {
    $.get('v1api.py?key={0}&op=get_personnel_names&ids={1}'.format(
        key, ids.join(',')
    ), function (data) {
        data = $.parseJSON(data);
...
xemo.core.getPaySystemInfo = function (key, ids, cb) {
    $.get('v1api.py?key={0}&ids={1}&op=getpaysysinfo'.format(
        key, ids.join(',')), function (data) {
        var data = $.parseJSON(data);
        if (!data) {
            cb(undefined);

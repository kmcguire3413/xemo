...
Date.prototype.shortString = function () {
    ...
    var mnth = new String(this.getMonth() + 1);
...
function enumeratehoursbyid(shifts, pps, ppe, id, pid) {
    var totalhours = 0.0;
    for (var i = 0; i < shifts.length; ++i) {
        ...
        if (shifts[i].allocatedfor != null) {
            if (shifts[i].allocatedfor.id == id && shifts[i].pid == pid) {
...
/*
    Returns the hour and fractional hour rounded to the
    ...
    while (need > 0) {
        ...
        for (var d = 0; d < ordered.length; ++d) {
            ...
            for (var x = 0; x < ordered[d].length; ++x) {
                ...
                /*
                    The problematic usage of hours that are not factors
                    of 0.25 hours. This prevents the scenario of trying to
                    grab extremely small fractions of hours. This needs to
                    be fixed when interpreting the calendar by rounding times
                    to the near 15 minute interval.
                */
                if (need <= 0.1) {
                    break;
                }

                var hours = shift.getTimeDuration() / 1000.0 / 60.0 / 60.0;
                ...
                if (result.length > 0) {
                    ...
                    var actually_got = picked.getTimeDuration() / 1000.0 / 60.0 / 60.0;
                    
                    console.log('need minus', actually_got, 'gothere', gothere);

                    need -= actually_got;             
                    dayneed -= actually_got;
                ...
                if (result.length > 0) {
                    for (var y = 0; y < result.length; ++y) {
                        if (result[y] != picked) {
                            ordered[d].push([
                                result[y].getTimeDuration() / 1000 / 60 / 60,
                                result[y],
                                shiftndx
                            ]);
                        }
...
function system_hourly_withcap(params) {
    ...
    var gothours = enumeratehoursbyid(shifts, cur_pp_start, cur_pp_end, allocationid, pid);
    if (gothours >= cap) {
        return {
            'total':   gothours,
            'shifts':  []
...
xemo.core.sopv3 = function (data, success, error, fail) {
...
xemo.core.paysys = {
    'DIRECTOR': function (params) {
        ...
        */
        ...
        console.log('@hourcap', ppinfo[1][0], hourcap);
    ...
    'DRIVER_8X5LUNCH': function (params) {
        var shifts = params.shifts;
        var ppstart = params.ppstart;
        var ppend = params.ppend;
        var calls = params.calls;
    ...
    for (var pid in paysysinfo['mapping']) {
        ...
        for (; cur_pp_start <= lastdayofrange; cur_pp_start.setDate(cur_pp_start.getDate() + 7), cur_pp_end.setDate(cur_pp_end.getDate() + 7)) {
            ...
            if (sysid == undefined || sysid == null) {
                master.warnings.push('The personnel({0}) named {1} did not have a pay system applied for the pay period of {2}, because none matching start and end was found. ppid:{3}'.format(
                    pid, pids[pid], cur_pp_start, ppid
                ));
                continue;
            }
...
xemo.core.shifts.checkvalid = function (shifts) {
    for (var x = 0; x < shifts.length; ++x) {
        if (!shifts[x].isValid()) {
            console.log('WHOA HORSEY.. FOR ' + JSON.stringify(shifts[x]));
        }
    }
}
...
xemo.core.textcalendar.parseLinesIntoShifts = function (
    ...
    for (var ndx in lines) {
        ...
        if (rng != null) {
            ...
            /*
                Rework the times to be on 15-minute boundaries
                since this helps a lot in the calculations. If
                not then further down the chain code starts having
                to deal with very odd fractional parts that are
                essentially difficult and expensive to accurately
                compute such as 59.0/60.0 minutes which is quite a
                long fractional part and easily to create mathematical
                inconsistencies with.
            */
            rng[0][2] = Math.round(rng[0][2] / 15.0) * 15.0;
            rng[1][2] = Math.round(rng[1][2] / 15.0) * 15.0;
            rng[0][0] = rng[0][1] + rng[0][2] / 60.0;
            rng[1][0] = rng[1][1] + rng[1][2] / 60.0;
...
Shift.prototype.countHoursBetweenHoursBetweenWeekdays = function(start_hour, end_hour, start_day, end_day) {
    var et = this.end;
    var sum = 0;

    var cur = new Date(this.start.getFullYear(), this.start.getMonth(), this.start.getDate(), start_hour);
    var max_hours_per_day = end_hour - start_hour;
    while (cur.getTime() < et.getTime()) {
        if (cur.getDay() >= start_day && cur.getDay() <= end_day) {
            var xt;
            if (this.start.getTime() > cur.getTime()) {
                xt = this.start.getTime();
            } else {
                xt = cur.getTime();
            }
            if (et.getTime() - xt > max_hours_per_day * 60 * 60 * 1000) {
                sum += max_hours_per_day;
            } else {

                sum += (et.getTime() - xt) / 1000 / 60 / 60;
            }
        }
        cur.setTime(cur.getTime() + (1000 * 60 * 60 * 24));
    }
...
Shift.prototype.isValid = function () {
    if (this.end.getTime() < this.start.getTime()) {
        console.log('shift end < start', this);
        return false;
    }
    if (this.end.getTime() == this.start.getTime()) {
        console.log('shift end == start', this);

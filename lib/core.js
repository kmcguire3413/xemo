        ...
        for (var i = 0; i < shifts.length; ++i) {
            ...
            if (shifts[i].start.getHours() == 6 && shifts[i].start.getDay() >= 1 && shifts[i].start.getDay() <= 5) {
                ...
                if (totalshifthours > need) {
                    ...
                    ++i;
                    $('#dbgarea').append('removed {0} hours from shift {1} to {2}<br/>'.format(need, shifts[i].start, shift_end));
                } else {
                    totalhours += totalshifthours;
                    $('#dbgarea').append('removed entire shift {0} to {1}<br/>'.format(shifts[i].start, shift_end));
                    // We are going to just remove the entire shift.
                    allocated.push({
                        'start':          shifts[i].start.clone(),
                        'end':            shifts[i].end.clone(),
...
function plugin_pay_calculator(pidattrib, calls, shifts, key, from, to, cfg, cb) {
    ...
    $('#dbgarea').append('incentive calculation lock for pay period dependancy set at ' + incentive_calculation_lock_for_payperiod + '<br/>');
    ...
    for (var i = 0; i < shifts.length; ++i) {
        ...
        if (shift.pid != null && ($.inArray(shift.pid, incentive_calulation_lock_for_payperiod_who) > -1)) {
            ...
            if (start_pp[0].getTime() != end_pp[0].getTime()) {
                $('#dbgarea').append('debug of {0} to {1}<br/>'.format(shift.start, shift.end));
                $('#dbgarea').append(' period #1 {0} to {1}<br/>'.format(start_pp[0], start_pp[1])); 
                $('#dbgarea').append(' period #2 {0} to {1}<br/>'.format(end_pp[0], end_pp[1]));
                ...
                ++i;
                $('#dbgarea').append('split shift for {0} into {1} .. {2} .. {3}<br/>'.format(
                    shift.name, shift.start, newshift.start, newshift.end
                ));
                $('#dbgarea').append('  {0}-{1}<br/>    {2}-{3}<br/>'.format(
    ...
    function splitshiftsby(shifts, by) {
        for (var i = 0; i < shifts.length; ++i) {
            if (splitshiftinplaceby(shifts, i, by)) {
                $('#dbgarea').append('splitshiftsby for {0} on {1} and {2}'.format(
    ...
    for (var i = 0; i < shifts.length; ++i) {
        var tmp = shifts[i].end.clone();
        tmp.setHours(0);
        var result = shifts[i].splitby(tmp);
        if (result != null) {
            shifts.splice(i, 1, result[0], result[1]);
            ++i;
        }
    }

    $('#dbgarea').append('<table>');
    for (var i = 0; i < shifts.length; ++i) {
        var shift = shifts[i];
        if (shift.allocatedfor != null) {
            foundnonnull = true;
        ...
        //$('#dbgarea').append('<tr><td> {0} </td><td> {1} </td><td> {2} </td><td> {3} </td><td> {4} </td></tr>'.format(
        ...
        if (shift.start.getDate() != lastday) {
            $('#dbgarea').append('<tr><td>' + line.join('') + '</td></tr>');
            line = [];
            lastday = shift.start.getDate();
        }

        var w = Math.round((((shift.end - shift.start) / 1000.0 / 60.0 / 60.0) / 24.0) * tw);

        line.push(
            '<span style="float: left; width: ' + w + 'px; background-color: ' + bg + ';">' + shift.name + '(' + shift.start.getHours() + '-' + shift.end.getHours() + ')' + '</span>'
        );
    ...
    $.get('v1api.py?key={0}&grp={1}&from_year={2}&from_month={3}&from_day={4}&to_year={5}&to_month={6}&to_day={7}&op=readcalendar'.format(
        ...
        for (var i = 0; i < out.length; ++i) {
            ...
            if (!mock_shift) {
                ...
                //    $('#dbgarea').append('@@@{0} {1} {2} {3} {4} ... {5} {6}<br/>'.format(out[i].year, out[i].month, out[i].day, out[i].start, out[i].end, tmp[0], tmp[1]));
                //}

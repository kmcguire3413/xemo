...
*/
xemo.core.getpayperiodidbydate = function (d, half) {
...
function plugin_pay_calculator(paysysinfo, pidattrib, calls, shifts, key, from, to, cfg, cb) {
    var master = {};
    ...
    // this specific date.
    incentive_calculation_lock_for_payperiod = xemo.core.getpayperiodidbydate(lastdayofrange)[0];
    xemo.core.log(10, 'incentive calculation lock for pay period dependancy set at ' + incentive_calculation_lock_for_payperiod + '<br/>');
    ...
    var tmp = xemo.core.getpayperiodidbydate(firstdayofrange);
    pp_start = tmp[0];
    pp_end = tmp[1];
    ...
    */
    for (var i = 0; i < shifts.length; ++i) {
        var shift = shifts[i];
        var result = xemo.core.getpayperiodidbydate(shift.start, 2);
        ...
        if (shift.pid != null && ($.inArray(shift.pid, incentive_calulation_lock_for_payperiod_who) > -1)) {
            var result = xemo.core.getpayperiodidbydate(shift.end, 2);
            var end_pp = result[1];
    ...
    for (var i = 0; i < incentive_calulation_lock_for_payperiod_who.length; ++i) {
        ...
        for (; cur_pp_start <= lastdayofrange; cur_pp_start.setDate(cur_pp_start.getDate() + 7), cur_pp_end.setDate(cur_pp_end.getDate() + 7)) {
            xemo.core.log(10, 'doing payperiod {0} to {1}<br/>'.format(cur_pp_start, cur_pp_end));
    ...
    xemo.core.log(10, 'doing capitalism incentive pay calculation ' + shifts.length + '<br/>');
    ...
    for (var i = 0; i < shifts.length; ++i) {
        ...
        if (shift.pid != null && ($.inArray(shift.pid, incentive_calulation_lock_for_payperiod_who) > -1)) {
            ...
            if (end > incentive_calculation_lock_for_payperiod) {
                ...
                xemo.core.log(10, 'name:{0} pid:{1} shift incurred incentive calculation lock for payperiod reasons of {2} to {3} to {4}<br/>'.format(
...
xemo.core.getPaySystemInfo = new function (key, ids, cb) {
